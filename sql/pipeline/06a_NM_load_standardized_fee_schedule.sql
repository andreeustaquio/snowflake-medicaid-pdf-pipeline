...
/*
Step 1: Preview rows that will be loaded (headers)
*/
...
SELECT
    ...
    CASE
        WHEN TRIM(
            GET(
                r.RAW_EXTRACTION_RESPONSE:response:fee_schedule:tax,
                code.index
            )::VARCHAR
        ) IN ('', 'None')
            THEN NULL
        ...
        NULLIF(
            TRIM(
                REPLACE(
                    REPLACE(
                        GET(
                            r.RAW_EXTRACTION_RESPONSE:response:fee_schedule:rate,
                            code.index
                        )::VARCHAR,
                        '$',
                        ''
    ...
    CASE
        WHEN TRIM(
            GET(
                r.RAW_EXTRACTION_RESPONSE:response:fee_schedule:pricing_note,
                code.index
            )::VARCHAR
        ) IN ('', 'None')
            THEN NULL
    ...
    CASE
        WHEN TRIM(
            GET(
                r.RAW_EXTRACTION_RESPONSE:response:fee_schedule:vfc,
                code.index
            )::VARCHAR
        ) IN ('', 'None')
            THEN NULL
        ...
        NULLIF(
            TRIM(
                REPLACE(
                    REPLACE(
                        GET(
                            r.RAW_EXTRACTION_RESPONSE:response:fee_schedule:rate_2,
                            code.index
                        )::VARCHAR,
                        '$',
                        ''
    ...
    -- Clean price start date
    TRY_TO_DATE(
        NULLIF(
            NULLIF(
                TRIM(
                    GET(
                        r.RAW_EXTRACTION_RESPONSE:response:fee_schedule:price_start_date,
                        code.index
                    )::VARCHAR
                ),
                ''

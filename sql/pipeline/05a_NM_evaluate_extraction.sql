...
SELECT
    COALESCE(
        SUM(
            ARRAY_SIZE(
            RAW_EXTRACTION_RESPONSE:response:fee_schedule:cpt_code
            )
        ),
    0
    ) AS TOTAL_ROWS_FROM_CPT_CODE
FROM RAW_EXTRACTION_RESULTS
WHERE DOCUMENT_ID = $DOCUMENT_ID
AND PARSE_RUN_ID = $PARSE_RUN_ID
AND EXTRACTION_VERSION = $EXTRACTION_VERSION
...
SELECT
    ...
    ARRAY_SIZE(
        RAW_EXTRACTION_RESPONSE:response:fee_schedule:tax
    ) AS TAX_COUNT,

    ARRAY_SIZE(
        RAW_EXTRACTION_RESPONSE:response:fee_schedule:rate
    ) AS RATE_COUNT,

    ARRAY_SIZE(
        RAW_EXTRACTION_RESPONSE:response:fee_schedule:pricing_note
    ) AS PRICING_NOTE_COUNT,

    ARRAY_SIZE(
        RAW_EXTRACTION_RESPONSE:response:fee_schedule:vfc
    ) AS VFC_COUNT,
    ...
    ARRAY_SIZE(
        RAW_EXTRACTION_RESPONSE:response:fee_schedule:rate_2
    ) AS RATE_2_COUNT,

    ARRAY_SIZE(
        RAW_EXTRACTION_RESPONSE:response:fee_schedule:price_start_date
    ) AS PRICE_START_DATE_COUNT
...
SELECT
    ...
    GET(
        r.RAW_EXTRACTION_RESPONSE
            :response:fee_schedule:tax,
        code.INDEX
    )::VARCHAR AS TAX,

    GET(
        r.RAW_EXTRACTION_RESPONSE
            :response:fee_schedule:rate,
        code.INDEX
    )::VARCHAR AS RATE,

    GET(
        r.RAW_EXTRACTION_RESPONSE
            :response:fee_schedule:pricing_note,
        code.INDEX
    )::VARCHAR AS PRICING_NOTE,

    GET(
        r.RAW_EXTRACTION_RESPONSE
            :response:fee_schedule:vfc,
        code.INDEX
    )::VARCHAR AS VFC,
    ...
    GET(
        r.RAW_EXTRACTION_RESPONSE
            :response:fee_schedule:rate_2,
        code.INDEX
    )::VARCHAR AS RATE_2,

    GET(
        r.RAW_EXTRACTION_RESPONSE
            :response:fee_schedule:price_start_date,
        code.INDEX
    )::VARCHAR AS PRICE_START_DATE
...
LATERAL FLATTEN(
    INPUT =>
        r.RAW_EXTRACTION_RESPONSE
            :response:fee_schedule:cpt_code
) AS code
...
SELECT
    r.PAGE_NUMBER,
    code.INDEX AS ROW_INDEX,
    code.VALUE::VARCHAR AS CPT_CODE
